...
export default function MobileLogin() {
  return (
    <div className="relative min-h-screen w-full bg-slate-950 flex flex-col">
      ...
      {/* HEADER */}
      <header className="relative z-50 flex items-center justify-between px-2.5 sm:px-4 py-3 sm:py-5">
        <div className="flex items-center gap-2.5">
          {/* BACK BUTTON */}
          <Link 
            href="/" 
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/5 backdrop-blur-md border border-white/10 text-white transition-all active:scale-90"
          >
            <ArrowLeft size={16} />
          </Link>
      ...
      <main className="relative z-20 flex-1 flex flex-col items-center justify-start px-2 sm:px-3 pt-8 sm:pt-14 pb-4 sm:pb-8">
        <div className="w-full max-w-[400px] animate-in fade-in zoom-in-95 duration-700">
          <div className="text-center mb-3 h-5 sm:h-6 flex items-center justify-center">
            <Typewriter 
              text={["Welcome back, BISU'an!", "Sign in to your portal", "Secure Digital Clearance"]} 
              speed={60}
              pause={3000}
              className="text-[9px] sm:text-[10px] font-bold text-white/50 uppercase tracking-[0.16em] sm:tracking-[0.2em]" 
            />
          </div>
          <LoginForm mobile />
        </div>
