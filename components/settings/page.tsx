...
export default function SettingsModal({ onClose, isOpen, defaultTab }: SettingsModalProps) {
  ...
  const [desktopModalSize, setDesktopModalSize] = useState({ width: 720, height: 560 });
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width >= 768) {
        const nextWidth = Math.min(720, Math.max(620, width - 64));
        const nextHeight = Math.min(560, Math.max(480, height - 72));
        setDesktopModalSize({ width: nextWidth, height: nextHeight });
        setPosition({ x: Math.max(12, width / 2 - nextWidth / 2), y: Math.max(12, height / 2 - nextHeight / 2) });
      } else {
        setPosition({ x: 0, y: 0 });
      }
    };
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 bg-white dark:bg-slate-950 z-[80] flex flex-col font-sans">
        <header className="p-3 border-b dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-950 sticky top-0">
            ...
            <nav className="space-y-1.5">
              {menuItems.map((item) => (
                <button
                  ...
                  className="w-full flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-900 rounded-xl active:bg-gray-100 dark:active:bg-slate-800"
                >
                  <div className="flex items-center gap-3">
