...
import {
  ...
  Clock, Building2, FileText, Edit3, Trash2, Settings2
} from "lucide-react";
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-5 md:p-8 rounded-2xl md:rounded-[2rem] hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-none transition-all group">
      <div className="flex flex-col lg:flex-row gap-6 lg:items-start">
        ...
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-sm ${
          ...
          {req.format === "Digital" ? <HardDrive size={26} /> : req.format === "Physical" ? <HandMetal size={26} /> : <Settings2 size={26} />}
        </div>

        {/* Main info */}
        <div className="flex-1 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-brand-500"><GraduationCap size={12} /> {req.targetYear}</span>
          ...
          <h3 className="text-xl md:text-2xl font-black text-slate-800 dark:text-slate-200 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{req.title}</h3>
        ...
        {req.format === "Physical" && (
          <div className="grid grid-cols-1 gap-3 p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-800 lg:w-72 shrink-0">
            ...
            <INFO_ROW icon={<Building2 size={12} className="text-brand-500" />} label="Office Location" value={req.officeLocation} />
        ...
        <div className="flex lg:flex-col gap-2 pt-4 lg:pt-0 border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-800 lg:pl-6">
          <button onClick={() => onEdit(req)} className="flex-1 lg:flex-none p-3.5 text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 hover:bg-brand-50 dark:hover:bg-brand-500/10 rounded-xl transition-all flex justify-center"><Edit3 size={20} /></button>
