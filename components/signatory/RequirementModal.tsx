...
const inp = "w-full p-4 bg-slate-50 dark:bg-slate-800/50 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 focus:border-brand-500 dark:focus:border-brand-500 rounded-md text-sm font-bold outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600";
const lbl = "text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 ml-1";
const smInp = "w-full p-3 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-200 border border-slate-200 dark:border-slate-700 focus:border-brand-500 rounded-md text-xs font-bold outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600";
...
export function RequirementModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      {/* PREVIEW PANEL */}
      {showPreview && (
        <div className="relative z-10 bg-brand-950 text-white w-full max-w-md rounded-md p-8 shadow-2xl mx-4 space-y-4 animate-in fade-in zoom-in-95 duration-200">
          ...
          {formData.format === "Physical" && formData.officeLocation && (
            <div className="bg-brand-900/50 rounded-md p-4 space-y-2 text-sm">
          ...
          <button onClick={() => setShowPreview(false)} className="w-full py-3 bg-brand-600 hover:bg-brand-500 rounded-md text-xs font-black uppercase tracking-widest transition-colors">Close Preview</button>
        </div>
      )}

      {/* CONFIRM DIALOG */}
      {showConfirm && (
        <div className="relative z-10 bg-white dark:bg-slate-900 w-full max-w-sm rounded-md p-8 shadow-2xl mx-4 text-center space-y-5 border border-slate-100 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setShowConfirm(false)} className="py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-md text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Cancel</button>
            <button onClick={handleConfirmed} disabled={saving} className="py-4 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white rounded-md text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2">
      ...
      {!showPreview && !showConfirm && (
        <form onSubmit={handleSubmitClick} className="relative bg-white dark:bg-slate-900 w-full max-w-2xl rounded-t-md md:rounded-md shadow-2xl flex flex-col max-h-[95vh] md:max-h-[90vh] overflow-hidden animate-in slide-in-from-bottom-8 duration-300 border border-transparent dark:border-slate-800">
          ...
          <div className="p-4 sm:p-5 md:p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setShowPreview(true)} className="flex items-center gap-1.5 px-3 py-2 bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 rounded-md text-[10px] font-black uppercase tracking-widest hover:bg-brand-100 dark:hover:bg-brand-500/20 transition-colors">
                <Eye size={13} /> Preview
              </button>
          ...
          <div className="p-4 sm:p-5 md:p-8 space-y-4 sm:space-y-5 md:space-y-6 overflow-y-auto">
            ...
            {/* Format toggle */}
            <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700">
              {formatOptions.map(type => (
                <button key={type} type="button" onClick={() => setFormat(type)}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-md text-xs font-black uppercase tracking-widest transition-all ${formData.format === type ? "bg-white dark:bg-slate-700 shadow-md text-brand-600 dark:text-brand-400 border border-slate-200 dark:border-slate-600" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"}`}>
            ...
            {formData.format === "Conditional" && (
              <div className="p-5 bg-brand-50/50 dark:bg-brand-500/5 rounded-md border border-brand-100 dark:border-brand-500/20 space-y-4 animate-in fade-in duration-200">
            ...
            <div className="p-5 bg-slate-50 dark:bg-slate-800/40 rounded-md border border-slate-200 dark:border-slate-700 space-y-4">
            ...
            {/* ── SECTION 3: STATUS ── */}
            <div className="p-5 bg-slate-50 dark:bg-slate-800/40 rounded-md border border-slate-200 dark:border-slate-700 space-y-3">
            ...
            {formData.format !== "Conditional" && (
              <>
                ...
                {!formData.allowFileUpload && (
                  <p className="text-[11px] font-bold text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-md px-3 py-2">
            ...
            {formData.format === "Conditional" && (
              <p className="text-[11px] font-bold text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-sky-500/10 border border-sky-200 dark:border-sky-500/20 rounded-md px-3 py-2">
                Conditional requirements are system-based: no student upload and no manual signatory review action.
              </p>
            )}
            {formError && (
              <p className="text-[11px] font-bold text-rose-700 dark:text-rose-300 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-md px-3 py-2">
                {formError}
              </p>
            )}
          </div>
          ...
          <div className="p-4 sm:p-5 md:p-8 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 shrink-0">
            <button type="submit" className="w-full py-4 bg-slate-900 dark:bg-brand-600 text-white rounded-md text-[11px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-slate-800 dark:hover:bg-brand-500 transition-all active:scale-95">
