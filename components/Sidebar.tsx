    ...
    <>
      ...
      `}>
              ...
              <div key={item.label} className="flex flex-col">
                {item.hasDropdown ? (
                  ...
                  >
                    ...
                    {(!isCollapsed || isMobileOpen) && (
                      <>
                        <span className={`flex-1 text-sm font-bold text-left ${isOpen || isActive ? "text-white" : "group-hover:text-brand-600 dark:group-hover:text-brand-400"}`}>
                          {item.label}
                        </span>
                ...
                {isOpen && (!isCollapsed || isMobileOpen) && (
                  <div className="ml-9 mt-1 space-y-1 border-l-2 border-slate-100 dark:border-slate-800 pl-3">
                    ...
                    {isActivityLogs && (
                      <>
                        <Link href={linkToRoute("Recent Logs")} onClick={() => { if (isMobileOpen && onMobileClose) onMobileClose(); }} className="block py-2 text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
                          Recent Logs
                        </Link>
                        <Link href={linkToRoute("System History")} onClick={() => { if (isMobileOpen && onMobileClose) onMobileClose(); }} className="block py-2 text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
                          System History
                        </Link>
                      </>
                    )}
        ...
        <div className="px-4 py-6 border-t border-slate-100 dark:border-slate-800 mt-auto">
          <Link 
            href="/helpandsupport"
            className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-brand-600 hover:text-white transition-all group"
          >
