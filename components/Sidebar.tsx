...
export function Sidebar({ role, activePage, onPageClick }: SidebarProps ) {
  ...
  if (role === "admin") topLinks = ["Home", "User Accounts", "Clearance Periods", "Clearance Progress"];
  if (role === "signatory") topLinks = ["Home", "Manage Requirements", "Student Clearance Status", "Review Submissions"];
  ...
  return (
    <aside className={`${isCollapsed ? "w-15" : "w-64"} bg-white border-r border-gray-400 h-full flex flex-col text-black transition-all duration-300 relative`}>
      ...
      <div className="flex flex-col overflow-y-auto px-2 md:px-4 space-y-1 md:space-y-2 h-full py-6">
        {topLinks.map((link) => {
          ...
          const isOpen = link === "Signatories" ? signatoriesOpen : activityLogsOpen;
            ...
            <div key={link} className="w-full">
              ...
              {isDropdown ? (
                <NavButton
                  ...
                  onClick={() => {
                    if (link === "Signatories") setSignatoriesOpen(!signatoriesOpen);
                    if (link === "Activity Logs") setActivityLogsOpen(!activityLogsOpen);
                  }}
                  showArrow
                ...
                <div className="mt-1 ml-6 space-y-1 border-l-2 border-purple-100 pl-2">
                  {signatoriesData.map((sig) => (
                    <Link key={sig.id} href={`/${role}/signatories/${sig.id}`}>
                      <div className={`px-3 py-2 rounded-lg cursor-pointer ${pathname === `/${role}/signatories/${sig.id}` ? 'bg-purple-50' : 'hover:bg-gray-50'}`}>
                        <p className="text-[11px] font-bold text-gray-800">{sig.role}</p>
                        <p className="text-[10px] text-gray-500 truncate mt-1">{sig.name}</p>
                      </div>
                    </Link>
                  ))}
                </div>
              ...
              {link === "Activity Logs" && isOpen && !isCollapsed && (
                <div className="mt-1 ml-6 space-y-1 border-l-2 border-purple-100 pl-2">
                  <Link href={linkToRoute("Recent Logs")}>
                    <NavButton label="Recent Logs" active={pathname === linkToRoute("Recent Logs")} />
                  </Link>
                  <Link href={linkToRoute("System History")}>
                    <NavButton label="System History" active={pathname === linkToRoute("System History")} />
                  </Link>
                </div>
              )}
            </div>
