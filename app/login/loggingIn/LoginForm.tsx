...
export default function LoginForm({ mobile = false }: LoginFormProps) {
  ...
  >
      ...
      <div className="mt-8 w-full space-y-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            className="h-12 w-full rounded-xl border border-gray-200 px-4 text-gray-900 outline-none transition focus:border-purple-500 focus:ring-2 focus:ring-purple-100"
            value={email}
            onChange={(e) => {
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="relative group">
            <input
              ...
              required
              placeholder="•••••••••••••"
              className="h-12 w-full rounded-xl border border-gray-200 px-4 pr-12 text-gray-900 outline-none transition focus:border-purple-500 focus:ring-2 focus:ring-purple-100"
              value={password}
              onChange={(e) => {
      ...
      <div className="mt-5 flex w-full items-center justify-between px-1">
        ...
        <button
          type="button"
          className="text-xs font-semibold text-purple-600 transition-colors hover:text-purple-800 hover:underline"
        >
          Forgot Password?
        </button>
      </div>
      ...
      <div className="mt-4 min-h-[24px] text-center">
        ...
        {error && <p className="animate-bounce text-sm font-medium text-red-500">{error}</p>}
      </div>
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="group relative mt-2 flex h-13 w-full items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br from-purple-600 to-indigo-500 p-3 font-heading font-bold tracking-wide text-white shadow-lg transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-70"
      >
        {isLoading ? (
