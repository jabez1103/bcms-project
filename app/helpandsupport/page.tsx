...
export default function HelpAndSupport() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-20 font-sans">
      ...
      <div className="relative bg-[#0f172a] text-white pt-20 pb-32 px-4 md:px-8 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 -mt-14 relative z-20">
        ...
        <div className="relative mb-12 group">
          ...
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-purple-500 transition-colors" size={24} />
          <input 
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Type your question (e.g., 'How to upload files?')..."
            className="w-full bg-white border border-slate-200 rounded-[2rem] py-6 pl-16 pr-8 text-slate-900 shadow-2xl shadow-slate-200/60 outline-none focus:ring-0 focus:border-purple-500/50 transition-all text-xl font-medium placeholder:text-slate-400"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredCategories.length > 0 ? (
            filteredCategories.map((item, idx) => (
              <div key={idx} className={`bg-white border border-slate-200 rounded-[2.5rem] p-10 transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200/80 group ${item.color}`}>
                <div className="flex items-start gap-8">
                  <div className="p-5 bg-slate-50 rounded-[1.5rem] group-hover:scale-110 group-hover:bg-white transition-all duration-500 shadow-sm border border-slate-100">
                    {item.icon}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-2xl font-black text-slate-800 tracking-tight">{item.title}</h3>
                      <ArrowUpRight size={20} className="text-slate-300 group-hover:text-purple-500 opacity-0 group-hover:opacity-100 transition-all" />
                    </div>
                    <p className="text-slate-500 text-sm leading-relaxed mb-8 font-medium italic opacity-80">{item.desc}</p>
                    
                    <div className="grid grid-cols-1 gap-3">
                      {item.links.map((link, i) => (
                        <button key={i} className="flex items-center justify-between w-full p-4 rounded-2xl bg-slate-50/50 border border-transparent hover:border-slate-200 hover:bg-white text-slate-600 hover:text-purple-600 font-bold text-sm transition-all group/link shadow-sm hover:shadow-md">
                          <span className="flex items-center gap-3">
                            <div className="w-1.5 h-1.5 rounded-full bg-slate-300 group-hover/link:bg-purple-500 transition-colors"></div>
                            {link}
                          </span>
                          <ChevronRight size={18} className="text-slate-300 group-hover/link:translate-x-1 group-hover/link:text-purple-500 transition-all" />
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            <div className="col-span-full py-20 text-center bg-white rounded-[3rem] border border-dashed border-slate-300">
               ...
               <p className="text-slate-500 font-bold text-lg">No results found for "{searchQuery}"</p>
               <button onClick={() => setSearchQuery("")} className="mt-2 text-purple-600 font-bold hover:underline">Clear search</button>
            </div>
          )}
        </div>

        {/* REFINED FOOTER CTA: High-performance Support Card */}
        <div className="mt-16 relative overflow-hidden bg-white border border-slate-200 rounded-[3rem] p-10 md:p-14 shadow-2xl shadow-slate-200/50">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
            <div className="max-w-md">
              <h2 className="text-3xl font-black text-slate-900 tracking-tighter mb-4">Still feeling stuck?</h2>
              <p className="text-slate-500 font-medium leading-relaxed">
                If our documentation didn't answer your question, please contact our support desk at the <span className="text-purple-600 font-bold">BISU-Clarin MIS Office</span>.
              </p>
            </div>
            
            <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
              <button className="group flex items-center justify-center gap-3 bg-slate-900 text-white px-10 py-5 rounded-[1.5rem] font-black hover:bg-slate-800 transition-all shadow-xl hover:-translate-y-1 active:scale-95">
                <MessageSquare size={22} className="group-hover:rotate-12 transition-transform" />
                Contact Admin
              </button>
              <button className="flex items-center justify-center gap-3 bg-slate-100 text-slate-700 px-10 py-5 rounded-[1.5rem] font-black hover:bg-slate-200 transition-all active:scale-95">
                Quick Chat
              </button>
            </div>
          </div>
